...
const TECHNICAL_DRAFT_PATTERN = /(?:https?:\/\/[^\s)]+|\/book\?[^\s)]+|\b(?:doctorId|appointmentId|fromAppointmentId|runId|traceId|actionId|messageDraftId)=[^\s&]+&?|\b[0-9a-f]{8}-[0-9a-f]{4}-[1-5][0-9a-f]{3}-[89ab][0-9a-f]{3}-[0-9a-f]{12}\b)/gi;

function sanitizeDraftText(value) {
  return String(value || '').replace(TECHNICAL_DRAFT_PATTERN, '').replace(/[ \t]{2,}/g, ' ').trim();
}
...
export default function AdminAgentOperationsPage({ user }) {
  ...
  useEffect(() => {
    ...
    const timer = setInterval(refreshPresentation, 750);
    return () => {
      cancelled = true;
      clearInterval(timer);
    };
  }, [selectedId, selected?.run?.status, paused, replay]);

...
export default function AdminAgentOperationsPage({ user }) {
  ...
  const approveAndRun = async () => {
    ...
    const response = await apiRequest(`/api/admin/agents/runs/${selected.run.id}/approve-and-run`, { method: 'POST', body: { actionIds } });
  ...
  return <section className={`page-shell agent-ops-page ${presentation ? 'agent-presentation-mode' : ''}`}>
    ...
    <div className="agent-ops-layout">
      <aside className="agent-run-list card"><div className="agent-panel-heading"><h2>Runs</h2><select value={filter} onChange={(event) => setFilter(event.target.value)} aria-label="Filter traces"><option value="all">All</option><option value="active">Active</option><option value="awaiting_approval">Awaiting approval</option><option value="executing">Executing</option><option value="completed">Completed</option><option value="failed">Failed</option><option value="real-ai">Real AI</option><option value="fallback">Fallback</option></select></div>{filteredTraces.length ? filteredTraces.map((trace) => <button type="button" className={`agent-run-card ${trace.id === selectedId ? 'selected' : ''}`} key={trace.id} onClick={() => { manualSelectionRef.current = true; setSelectedId(trace.id); setReplay(false); }}><span className="agent-run-card-top"><strong>{agentLabel(trace.agentType)}</strong><span className={`agent-status ${trace.status}`}>{trace.status.replaceAll('_', ' ')}</span></span><span>Trace {shortId(trace.id)} · Appt {shortId(trace.appointmentId)}</span><span>{trace.run?.plan?.model || 'Provider pending'} · {trace.run?.plan?.fallbackUsed ? 'Deterministic fallback' : trace.run?.plan ? 'Real AI' : 'Planning'}</span><small>{utcDateTime(trace.updatedAt)}</small></button>) : <p className="muted">No persisted traces yet. Trigger an agent to begin.</p>}</aside>
      <main className="agent-ops-main">{selected ? <>
        <article className="card agent-selected-summary"><div><p className="kicker">{replay ? 'Historical replay' : 'Selected trace'}</p><h2>{agentLabel(selected.agentType)}</h2><p className="muted">Trace {presentation ? masked(selected.id) : selected.id} · Run {presentation ? masked(selected.run?.id) : selected.run?.id || (selected.status === 'deduplicated' ? 'existing run reused' : 'not linked yet')}</p></div><div className="agent-selected-badges"><span className={`agent-status ${selected.status}`}>{selected.status.replaceAll('_', ' ')}</span><span>{selected.presentation?.outcome === 'existing_run_reused' ? 'Existing run reused' : selected.run?.plan?.model || (selected.presentation?.isTerminal ? 'No model call' : 'Model pending')}</span><span>{selected.run?.plan?.fallbackUsed ? 'Fallback used' : selected.run?.plan ? 'Real AI' : selected.status === 'awaiting_approval' ? 'Human approval required' : selected.presentation?.outcome === 'existing_run_reused' ? 'No duplicate AI request' : '—'}</span></div>{user?.role === 'admin' && selected.agentType === 'no_show_recovery' && selected.run?.status === 'awaiting_approval' && !replay ? <div className="agent-plan-controls"><button type="button" onClick={approveAndRun} disabled={controlBusy}>{controlBusy ? 'Starting…' : 'Approve and Run'}</button><button type="button" className="ghost" onClick={rejectRun} disabled={controlBusy}>Reject</button></div> : null}</article>

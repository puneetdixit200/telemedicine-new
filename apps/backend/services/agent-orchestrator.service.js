...
async function planReservedNoShowRun({ appointmentId, actor, input = {}, traceContext, reservedRun: existingReservedRun = null }) {
  ...
  const plan = { ...plannerResult.plan, generationSource: plannerResult.plan?.fallbackUsed ? (plannerResult.plan.generationSource || 'deterministic_localized_template') : 'openrouter', languageCode: language.code, languageName: language.name, languageNativeName: language.nativeName, languageScript: language.script, languageDirection: language.direction, languageSource: language.source, languageFallbackUsed: language.fallbackUsed };
